...
async function render() {
  ...
  let route=raw || (session.user?'overview':'home');
  if (session.user && ['home','login','track'].includes(route)) route='overview';
  if (!session.user && ['overview','projects','project','revisions','team','settings'].includes(route)) route='login';
  if (session.user) projects=await api('/projects');
  if (session.user?.role==='owner' && ['team','project'].includes(route)) team=await api('/team');
  let content;
  if (route==='home') content=homePage();
  else if (route==='request') content=requestPage(arg?decodeURIComponent(arg):'');
  else if (route==='login') content=loginPage();
  else if (route==='overview') content=overviewPage();
  else if (route==='projects') content=projectsPage();
  else if (route==='revisions') content=revisionsPage();
  else if (route==='team'&&session.user?.role==='owner') content=teamPage();
  else if (route==='settings') content=settingsPage();
  else if (route==='project'||(route==='track'&&session.projectId)) { current=await api('/projects/'+(route==='project'?arg:session.projectId)); content=detailPage(current); }
  else if (route==='track') content=trackForm();
  else content=heading('Page not found','Use the navigation to return to your workspace.');
